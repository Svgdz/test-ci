...
const confirmSchema = z.object({
  ...
  type: z.enum(['signup', 'recovery', 'invite', 'magiclink', 'email', 'email_change']),
  next: z.string().url(),
})

const normalizeOrigin = (origin: string) => origin.replace('www.', '').replace(/\/$/, '')
...
export async function GET(request: NextRequest) {
  ...
  if (!result.success) {
    ...
    return encodedRedirect('error', dashboardSignInUrl.toString(), 'Invalid Request')
  }
  ...
  const isDifferentOrigin =
    supabaseRedirectTo &&
    normalizeOrigin(new URL(supabaseRedirectTo).origin) !== normalizeOrigin(dashboardUrl.origin)

  l.info({
    key: 'auth_confirm:init',
    context: {
      supabase_token_hash: supabaseTokenHash ? `${supabaseTokenHash.slice(0, 10)}...` : null,
      supabaseType,
      supabaseRedirectTo,
      isDifferentOrigin,
  ...
  if (isDifferentOrigin) {
    throw new Error(redirect(supabaseClientFlowUrl.toString()))
  }
  ...
  try {
    ...
    if ('message' in sE && typeof sE.message === 'string' && sE.message.includes('NEXT_REDIRECT')) {
      throw e
    }
    ...
    return encodedRedirect('error', dashboardSignInUrl.toString(), 'Invalid Token')
  }
}
