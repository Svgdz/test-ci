...
import { getTimeoutMsFromUserMessage, USER_MESSAGES } from '@/configs/user-messages'
import { AuthFormMessage, AuthMessage } from '@/components/auth/form-message'
...
export default function ForgotPassword() {
  ...
  useEffect(() => {
    if (
      message &&
      (('success' in message && message.success) || ('error' in message && message.error))
    ) {
  ...
  const handleBackToSignIn = () => {
    ...
    const currentSearchParams = email ? `?email=${encodeURIComponent(email)}` : ''
    window.location.href = `${AUTH_URLS.SIGN_IN}${currentSearchParams}`
  }
    ...
    <div className='flex w-full flex-col'>
      ...
      >
        ...
        <Input
          ...
          required
        />
        {/* Button does not support 'loading' prop, so handle loading state differently */}
        <Button type='submit' disabled={isExecuting} aria-busy={isExecuting}>
