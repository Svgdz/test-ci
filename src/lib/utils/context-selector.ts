...
import { getEditExamplesPrompt, getComponentPatternPrompt } from '../../server/agent/edit-example'
...
**ALWAYS CHECK App.tsx FIRST** to understand what components exist and how they're imported!
...
1. First check if Header.tsx exists
...
/**
 * Build edit instructions based on edit type
 */
...
- If user says "change the button color to blue", change ONLY that button's color
- DO NOT refactor, reorganize, or "improve" other parts of the code
- DO NOT change imports, exports, or component structure unless specifically requested
- DO NOT add new features or functionality unless explicitly asked
- Preserve ALL existing functionality, props, state, and event handlers
- Return the COMPLETE file with only the targeted change
 ...
 */
function buildComponentRelationships(files: string[], manifest: FileManifest): string {
...
${contextFiles
  ...
  if (editIntent.type === EditType.UPDATE_COMPONENT || editIntent.type === EditType.ADD_FEATURE) {
 ...
 */
export function selectFilesForEdit(userPrompt: string, manifest: FileManifest): FileContext {
  ...
  // App.tsx is most important - shows component structure
  const appFile = allFiles.find((f) => f.endsWith('App.tsx') || f.endsWith('App.tsx'))
...
export function formatFilesForAI(
  ...
  sections.push(
    'The file MUST include ALL imports, ALL functions, ALL tsx, and ALL closing tags.\n\n'
  )
