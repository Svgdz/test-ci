...
function extractComponentInfo(content: string, filePath: string): ComponentInfo | undefined {
  ...
  const hastsx = /<[A-Z]\w*|<[a-z]+\s+[^>]*\/?>/.test(content)
  if (!hastsx && !content.includes('React')) return undefined
 ...
 */
function determineFileType(filePath: string, content: string): FileInfo['type'] {
...
/**
 * Parse a TypeScript/tsx file to extract imports, exports, and component info
 */
export function parseTypeScriptFile(content: string, filePath: string): Partial<FileInfo> {
  ...
  for (const [filePath, fileInfo] of Object.entries(files)) {
    if (fileInfo.componentInfo) {
      ...
      tree[componentName] = {
        ...
        importedBy: [],
        type:
          fileInfo.type === 'page' ? 'page' : fileInfo.type === 'layout' ? 'layout' : 'component',
      }
    }
  }
